<template>
  <banner />
  <myheader :img="img" :header="$t('category.headertitle') + ' ' + this.description.title" :text="$t('category.headertext')"
    :btns="[]" :logo="description.logo" />
  <v-container>
          ...
          <v-card class="card">
            ...
            <v-sheet class="card-body">
              ...
              <router-link :to="{
                name: 'lessons',
                params: {
                  academy: description.title,
                  category: category.id,
                  academyId: this.$route.params.academy
                }
              }" class="btn hvr-bounce-to-bottom">{{
                                  $t('category.gotocourses') }}</router-link>
            </v-sheet>
          </v-card>
        </v-col>
...
import description from '@/components/DescriptiContextRow.vue'
...
export default {
  ...
  , methods: {
    adtergetdetail(param) {
      ...
      var desc =''
      if (param.Data.describle != '')
        desc = param.Data.describle
      if (param.Data.address != '')
      {
        if(desc !='') desc += '<br>'
        desc += param.Data.address
      }
       
        if (param.Data.Phone != '')
        desc += '<br>'+param.Data.Phone
